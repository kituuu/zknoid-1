...
import { PokerLobby, PokerLobbyManager } from './PokerLobby';
...
@runtimeModule()
export class PokerMatchMaker extends PokerLobbyManager {
  // Round => pending lobby
  @state() public pendingLobby = StateMap.from<UInt64, PokerLobby>(
    UInt64,
    PokerLobby,
  );

  // User => session. 1 active session key for user
  @state() public userToSession = StateMap.from<PublicKey, PublicKey>(
    PublicKey,
    PublicKey,
  );
  ...
  private joinPendingLobby(lobbyId: UInt64): PokerLobby {
    const sender = this.transaction.sender.value;
  ...
  private flushPendingLobby(
    ...
    this.pendingLobby.set(
      ...
      Provable.if(
        shouldFlush,
        PokerLobby,
        PokerLobby.default(pendingLobyId),
        lobby,
      ) as PokerLobby,
    );
