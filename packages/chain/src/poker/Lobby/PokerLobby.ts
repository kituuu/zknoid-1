...
export class PokerLobby extends Struct({
  id: UInt64,
  ...
  static default(id: UInt64): PokerLobby {
    return new PokerLobby({
      id,
...
@runtimeModule()
export class PokerLobbyManager extends RuntimeModule<LobbyManagerConfig> {
  @state() public activeGameId = StateMap.from<PublicKey, UInt64>(
    PublicKey,
    UInt64,
  );
  @state() public activeLobby = StateMap.from<UInt64, PokerLobby>(
    UInt64,
    PokerLobby,
  );
  ...
  @runtimeMethod()
  public createLobby(): PokerLobby {
    return this._addLobby(PokerLobby.default(UInt64.zero), Bool(true));
  }
  ...
  public joinLobby(lobbyId: UInt64): void {
    const lobby = this.activeLobby
      .get(lobbyId)
      .orElse(PokerLobby.default(lobbyId));
    this._joinLobby(lobby);
    this.activeLobby.set(lobbyId, lobby);
  }

  protected _joinLobby(lobby: PokerLobby): void {
  ...
  protected _addLobby(lobby: PokerLobby, shouldUpdate: Bool): PokerLobby {
   ...
   */
  public initGame(lobby: PokerLobby, shouldInit: Bool): UInt64 {
