...
export default function NewArkanoidCompetitionPage() {
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      if (
        !values.competitionFrom ||
        !values.competitionTo ||
        (values.preregistrationEnabled &&
          new Date(values.competitionFrom).getTime() <=
            new Date(values.preregistrationTo).getTime())
      ) {
        errors.competitionFrom = '';
        errors.competitionTo = '';
      }

      if (!values.preregistrationFrom || !values.preregistrationTo) {
        errors.preregistrationFrom = '';
        errors.preregistrationTo = '';
      }

      if (values.participationFee < 0) {
      ...
      if (!values.funding || values.funding < 0) {
        errors.funding = '';
      }
  ...
  const createCompetition = async () => {
    ...
    if (await bridge(BigInt(formik.values.funding) * 10n ** 9n))
      ...
      () => {
        ...
        */
        ...
        let competition = Competition.from(
          ...
          // description,
          formik.values.seed,
          formik.values.preregistrationEnabled,
          new Date(formik.values.preregistrationFrom).getTime(), // preregStartTime
          new Date(formik.values.preregistrationTo).getTime(), // preregEndTime
          new Date(formik.values.competitionFrom).getTime(), // competitionStartTime
          new Date(formik.values.competitionTo).getTime(), // competitionEndTime
          formik.values.funding,
          formik.values.participationFee
        );
  ...
  useEffect(() => {
    if (!isRandomSeed && formik.values.seed.toString().length > 13)
                ...
                <div className={'flex w-full flex-col gap-2'}>
                  <div className={'flex flex-row justify-between'}>
                    ...
                    <div className={'flex w-[260px] flex-col gap-2'}>
                      ...
                      {formik.values.seed.toString().length > 13 ? (
                        <Popover
                  ...
                  <div className={'flex flex-row gap-4'}>
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          Preregistration allows to enable dates when users need
                          to pre-register and pay the competition fee
                        </div>
                      </div>
                    </Popover>
                  </div>
